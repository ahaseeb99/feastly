													...
													<div class="card p-4 pl-5 prepaid-cards @if ($prepaid->featured) price-card-border @endif">
														...
														<div class="plan prepaid-plan">
															...
															<div class="credits-box">
																@if ($prepaid->words != 0) <p class="fs-12 mt-2 mb-0"><i class="fa-solid fa-check fs-14 mr-2 text-success"></i> {{ __('Credits Included') }}: <span class="ml-2 font-weight-bold text-primary">{{ number_format($prepaid->words) }}</span></p>@endif
																 
															</div>
															<div class="text-center action-button mt-2 mb-2">
																<a href="#" class="btn mt-4 btn-primary-pricing choose-plan-btn" 
																
																data-price="{{ number_format((float)$prepaid->price, 2) }}" data-words-count="{{ number_format($prepaid->words) }}"
																
																data-plan-id="{{ number_format($prepaid->id) }}">{{ __('Select Package') }}</a> 
															</div>																								                                                                          
														</div>							
													</div>	
												</div>							
